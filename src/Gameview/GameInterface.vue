<script>
  ...
  export default {
    ...
    setup(props){
        ...
        //const gameinfo = JSON.parse(route.query.gameinfo)
        const rawdata = JSON.parse(route.query.gameinfo)
        const gameinfo = rawdata.split(",");
        const userid = parseInt(gameinfo[0], 10); 
        const difflv = parseInt(gameinfo[1], 10); 
        const awardDensity = parseInt(gameinfo[2], 10)
        const enemyDensity = parseInt(gameinfo[3], 10)
        let maxFireReady = 0
        let doubleScore = 0
        let bulletLv = 0
        ...
        const postCredits = async() =>{
            try{
            const response = await gameAPI.postCredits(userid,score.value,coupon.value)     
            console.log([userid,score.value,coupon.value])
            }catch(err){
                console.log(err)
            }
        }
            ...
            pushAirplaneTimer.value = setInterval(() => {
                ...
                if (airRandom < 1) {
                    ...
                    if (idx > -1) {
                        ...
                        airplaneList.value.push(new ShootAirplane(90, 90, x, 0))
                    }
                    ...
                    if (airRandom < 2) {
                        ...
                        if (idx > -1) {
                            ...
                            airplaneList.value.push(new FastAirplane(60, 90, x, 0))
                        }
                    ...
                    airplaneList.value.forEach((ele, ind) => {
                        if (ele.shootBy(element)) {
                            ...
                            if (ele.isDie()) {
                                ...
                                if(doubleScore == 1){score.value += ele.getScore();}
            ...
            awardTimer.value = setInterval(() => {
                awardList.value.forEach((element, index) => {
                    ...
                    if (hero.value.hitBy(element)) {
                        ...
                        if(element.getType() == 4) {coupon.value = coupon.value+1}
        ...
        return{
            ...
            enemyBulletList,
            maxFireBulletList,
            score,difficultLv,power,pause,life,coupon,
            startTimer,
            pauseTimer,
            pushMaxFire,
